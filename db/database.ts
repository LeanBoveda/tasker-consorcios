...
async function initializeDatabase() {
  ...
  for (const table of ["users", "consorcios", "tasks", "automatic_intake"]) {
    const tableInfo = await db.prepare(`PRAGMA table_info(${table})`).all<{ name: string }>();
    if (!(tableInfo.results ?? []).some((column) => column.name === "workspace_id")) {
      await db.prepare(`ALTER TABLE ${table} ADD COLUMN workspace_id TEXT NOT NULL DEFAULT 'main'`).run();
    }
  }
  await db.batch([
    db.prepare("DROP INDEX IF EXISTS consorcios_name_unique"),
    db.prepare("CREATE UNIQUE INDEX IF NOT EXISTS consorcios_workspace_name_unique ON consorcios (workspace_id, name)"),
    db.prepare("DROP INDEX IF EXISTS automatic_intake_source_external_unique"),
  ]);

  const legacyBuildings = await db.prepare(`SELECT DISTINCT building, workspace_id
    FROM tasks WHERE consortium_id IS NULL AND trim(building) <> ''`).all<{ building: string; workspace_id: string }>();
  for (const row of legacyBuildings.results ?? []) {
    let consortium = await db.prepare("SELECT id FROM consorcios WHERE name = ? COLLATE NOCASE AND workspace_id = ?")
      .bind(row.building, row.workspace_id).first<{ id: string }>();
    if (!consortium) {
      const id = crypto.randomUUID();
      const now = Date.now();
      await db.prepare(`INSERT INTO consorcios (id, name, address, notes, created_at, updated_at, workspace_id)
        VALUES (?, ?, '', '', ?, ?, ?)`).bind(id, row.building, now, now, row.workspace_id).run();
      consortium = { id };
    }
    await db.prepare("UPDATE tasks SET consortium_id = ? WHERE consortium_id IS NULL AND building = ? AND workspace_id = ?")
      .bind(consortium.id, row.building, row.workspace_id).run();
  }
